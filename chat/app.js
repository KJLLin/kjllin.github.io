...
let isSessionInitialized = false; // 会话初始化状态标记，避免重复初始化
// 全局通道和定时器
let msgChannel = null;
...
// ====================== 核心修复：会话校验逻辑 ======================
// 校验当前会话是否有效（仅会话初始化完成后调用）
async function checkSessionValid() {
  try {
    // 边界情况：未登录/会话未初始化，直接返回无效
...
async function doLogin() {
  try {
    ...
    // 仅做账号密码登录，会话处理统一放在handleAuthChange里
...
async function handleAuthChange(event, session) {
  try {
    ...
    console.log("用户登录成功，开始初始化会话，用户ID：", currentUser.id);

    // 1. 先查询用户信息，确保users表有记录
    let userInfo = null;
    try {
      const { data, error } = await sb
        .from("users")
    ...
    // 2. 核心修复：如果users表没有记录，手动补插入（兼容新用户/触发器失败的情况）
    if (!userInfo) {
    ...
    // 4. 核心修复：生成新的会话Token，更新到数据库
    console.log("生成新的会话Token");
